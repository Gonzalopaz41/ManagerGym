...
import { deleteRoutineThunk } from '../slice/routines.thunk';
...
const DeleteRoutineDialog = ({ open, onClose, routine }: Props) => {
  ...
  const handleConfirm = async () => {
    ...
    const action = await dispatch(deleteRoutineThunk(routine.id));
    setSubmitting(false);
    ...
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-[#111111] border border-[#222222] rounded-[8px] p-5 z-50 focus:outline-none">
          ...
          <p className="text-xs text-[#444444] mb-5">
            Se borran también todos sus días y ejercicios. Esta acción no se puede deshacer.
          </p>
